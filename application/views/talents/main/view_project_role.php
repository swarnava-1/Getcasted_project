...
$(document).ready(function(){
	...
	function apply(project_id,talent_id,role_id){
			$.ajax({
			url: '<?php echo base_url();?>talent/talent_details/apply_talent_id',
	...
	function schedule(project_id,talent_id,role_id,shootoption){
		...
		$.ajax({
			url: '<?php echo base_url();?>talent/talent_details/get_slot',
		...
		$.ajax({
			url: '<?php echo base_url();?>talent/talent_details/get_slot_date',
			data: {'project_id': project_id,'role_id':role_id}, // change this to send js object
			type: "post",
			success: function(data){
				var obj = jQuery.parseJSON(data);
	...
	function selftapesubmit(project_id,talent_id,role_id){
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 pageTitle">Profile match percentage</h1>
    </div>
    <div class="card shadow mb-4">
        <div class="card-body">
        <div class="table-responsive">
                                <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
											<th>Profile Match</th>
											<th></th>
                                            <th>Self Tape Requst</th>
											<?php $result = $this->project_model->get_selftape_data($this->input->get('project_id'),$this->session->userdata('user_id'),$this->input->get('id'));if($result[0]['request_status']==2){?>
											<th>Schedule Audition</th>
											<?php } if($result[0]['request_status']==1){?>
											<th>Schedule Shoot</th>
											<?php } ?>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
											<td><?php 
													...
													$talent_ethnicity = $talent[0]['ethnicity'];
													...
													$talent_singing = $talent[0]['singing'];
													...
													$talent_feet = $talent[0]['feet'];
													
													$role_inches = $role_by_id[0]['inches'];
													$talent_inches = $talent[0]['inches'];
													...
													$talent_dancing = $talent[0]['dancing'];
													...
													$talent_training = $talent[0]['training'];
													...
													$talent_citizenship = $talent[0]['citizenship_or_visa_documents'];
									...
									<?php } ?></div>
											...
											$result = str_replace( array("T"), ' ', $role_by_id[0]['self_tape_deadline']);
											?>
											<?php $selftape=$this->project_model->get_selftape_data($this->input->get('project_id'),$this->session->userdata('user_id'),$this->input->get('id'));if(!empty($selftape) && ($selftape[0]['submit_selftape']==0) && (date($today) < $result)){?>
											<button type="button" class="btn btn-success" data-toggle="modal" data-target="#submitselftape" onclick="selftapesubmit('<?php echo $this->input->get('project_id')?>','<?php echo $this->session->userdata('user_id');?>','<?php echo $this->input->get('id');?>')" >Self Tape Submit</button>
											<?php } else {?>
											<button type="button" class="btn btn-success" disabled>Self Tape Submit</button>
											<?php } ?>
											</td>
											<?php $result = $this->project_model->get_selftape_data($this->input->get('project_id'),$this->session->userdata('user_id'),$this->input->get('id'));
											if($result[0]['request_status']==2 && $result[0]['audition_schedule']==0){?>
											<td>
												<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal" onclick="schedule('<?php echo $this->input->get('project_id')?>','<?php echo $this->session->userdata('user_id');?>','<?php echo $this->input->get('id');?>','2')">Schedule Audition</button>
											</td>
											<?php }elseif($result[0]['request_status']==2 && $result[0]['audition_schedule']==2){?> 
											<td>
												<?php echo '<strong>Date:-'.$result[0]['slot_date'].' '.'Time:-'.$result[0]['slot_time'].'</strong>';} ?>
											</td>
											<?php if($result[0]['request_status']==1 && $result[0]['audition_schedule']==0 || $result[0]['request_status']==1 && $result[0]['audition_schedule']==2){?>
											<td>
												<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal" onclick="schedule('<?php echo $this->input->get('project_id')?>','<?php echo $this->session->userdata('user_id');?>','<?php echo $this->input->get('id');?>','1')">Schedule Shoot</button>
											</td>
											<?php }elseif($result[0]['request_status']==1 && $result[0]['audition_schedule']==1){;?>
											<td>
											<?php echo '<strong>Date:-'.$result[0]['slot_date'].' '.'Time:-'.$result[0]['slot_time'].'</strong>';} ?>
											</td>	
                                        </tr>						
				...
				<!-- Modal -->
				<div class="modal fade" id="submitselftape" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
					<div class="modal-dialog" role="document">
						<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="exampleModalLabel">Submit Your Self Tape</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close" onClick="window.location.reload();">
							<span aria-hidden="true">&times;</span>
							</button>
						</div>
						<div class="modal-body">
						<form action="<?php echo base_url()?>talent/talent_details/submit_self_tape" method="post" enctype="multipart/form-data">
							<div class="modal-body">
								...
								<div class="form-group row">
									<div class="col-sm-12 mb-3 mb-sm-0">
										...
										<span>Upload Self Tape</span>
                                        	<input type="file" class="" id="exampleFirstName" name="self_tape" required/>
                                    	</div>
                                    </div>   
                                </div>
							</div>
				...
				<div class="modal-dialog" role="document">
					...
					<form action="<?php echo base_url()?>talent/talent_details/scheduleaudition" method="post">
						<div class="modal-body">
