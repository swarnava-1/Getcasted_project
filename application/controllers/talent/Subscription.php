...
class Subscription extends CI_Controller { 
    ...
    public function index(){ 
        ...
        if($this->input->post('stripeToken')){ 
            ...
            if($paymentID){ 
                redirect('talent/subscription/payment_status/'.$paymentID); 
            }else{ 
                ...
                if($plan){ 
                    ...
                    if($subscription){ 
                        ...
                        if($subscription['status'] == 'active'){ 
                            ...
                            $user_id = $this->userID;
                            $checkuser = $this->user->checkSubscriptionbyId($user_id);
                            ...
                            //echo '<pre>';print_r($subscripUpdateData);die;
                            $updatesubscription = $this->user->updatePlan($subscripUpdateData,$this->userID); 
                            $subscription_id = $checkuser[0]['id'];
                            // Update subscription id in the users table  
                            if($subscription_id && !empty($this->userID)){ 
                                $data = array('subscription_id' => $subscription_id); 
                                $update = $this->user->updateUser($data, $this->userID); 
